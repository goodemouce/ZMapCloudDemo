...
namespace ZMap
{
    ...
    export class Demo 
    {
        ...
        private _header : HTMLElement;
        private _content : HTMLElement;
        constructor(name:string, header:HTMLElement, content: HTMLElement)
        {  
            this._name = name;
            this._header = header;
        ...
        addByName<K extends keyof HTMLElementTagNameMap>(name:K, options?: ElementCreationOptions)
        {
        ...
        addButton(name:string, callback:(e:Event) => void)
        {
        ...
        addInput(type:string, id?:string, callback?:(input:HTMLInputElement, e:Event) => void):HTMLInputElement
        {
        ...
        addInput2(option:{
            ...
            title?:string; 
            id?:string;
            callback?:(input:HTMLInputElement, e:Event) => void}):HTMLInputElement
        {
        ...
        addRange(
            ...
            max?:number,
            value?:number,
            callback?:(input:HTMLInputElement, e:Event) => void
        ):HTMLInputElement
        {
        ...
        })
        {
            ...
            if (CZMAP.defined(option.default)) 
            {
                const bt = this.addButton('↺', (e) => {
                    ...
                    if (input)
                        input.value = option.default as any;
                    if (option.callback) 
                        option.callback(range, e);
                });
                bt.remove();
        ...
        addSelect(list:string[]|{name:string, value:string}[], callback?:(e:Event, select:HTMLSelectElement) => void):HTMLSelectElement
        {
